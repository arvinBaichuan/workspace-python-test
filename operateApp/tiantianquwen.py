...
class TianTianQuWen(object):
    ...
    def run(self):
        # 0.打开应用
        # init_device(platform="Android", uuid=device_id)
        # home()
    ...
    def watch_news_recommend(self):
        ...
        """
        # 1.推荐的所有新闻元素
        lv_elements = poco(btn_news_root).children()
        ...
        for news_element in lv_elements:
            ...
            sleep(2)
            # 评论拿金币和发表按钮
            comments_with_coins = poco('com.songheng.eastnews:id/lm')
            submit_btn_element = poco("com.songheng.eastnews:id/m6").offspring('com.songheng.eastnews:id/vw')
            # 记录时长的标识
            # 不存在就直接返回
            red_coin_element = poco('com.songheng.eastnews:id/aos')
    ...
    def __video(self):
        ...
        :return:
        """
        poco(btn_video).click()
        while True:
            # 视频列表
            poco(btn_video_root).wait_for_appearance()
            sleep(2)
            self.__read_key_news()
            video_elements = poco(btn_video_root).children()
    ...
    def __read_key_news(self):
        ...
        """
        # 对于弹出来的要闻对话框，需要处理
        key_news_element = poco(name='com.songheng.eastnews:id/xj', text='立即查看')
    ...
    def play_video(self):
        ...
        while True:
            # 视频播放结束或者超过30秒
            scale_element = poco('com.songheng.eastnews:id/aq0')
    ...
    def get_top_title_coin(self):
        ...
        :return:
        """
        get_coin_element = poco(name=btn_receive, text="领取")
    ...
    def __back_to_list(self):
        ...
        """
        print('准备回到首页')
        while not poco(btn_news_root).exists():
            print('回退一次')
            self.__back_keyevent()
